  ...
  constructor() {
    ...
    try {
      this.tabs.forEach((tab, index): void => {
        ...
        const tabsNav = tab.querySelector(
          "[data-tabs-nav]",
        ) as HTMLUListElement;
        const tabsBtns: NodeListOf<HTMLButtonElement> =
          tabsNav.querySelectorAll("button");
        const tabsPanels: NodeListOf<HTMLDivElement> = tab.querySelectorAll(
          "[data-tabs-panels]>*",
        );
  ...
  init(): void {
    this.tabsDetails.forEach((tabDetails): void => {
      ...
      tabDetails.tabsBtns.forEach((tabsBtn, index): void => {
        ...
        tabsBtn.setAttribute(
          "id",
          `tab${tabDetails.tabsIndex + 1}-${index + 1}`,
        );
      });
      ...
      tabDetails.tabsPanels.forEach((tabsPanel, index): void => {
        ...
        tabsPanel.setAttribute(
          "aria-labelledby",
          tabDetails.tabsBtns[index].id,
        );
      });
